...
### Bug Fixes
- Stopped shipping leftover Android build artifacts inside the package. Previous releases packed a versioned `pushwoosh-unityplugin-X.Y.Z.aar` and a `pushwoosh-resources.androidlib` folder without `.meta` files, which caused Unity to throw red "asset will be ignored" errors on import. The package now contains a single `pushwoosh-unityplugin.aar` with its `.meta`.
