...
class BuildScript {
	...
	[MenuItem("Custom/Build/Build WP8")]
	static void PerformWP8Build() {
		string buildPath = System.IO.Directory.GetCurrentDirectory() + "/build-wp8";
		CreateDirectory (buildPath);
		GenericBuild(SCENES, buildPath, BuildTarget.WP8Player, BuildOptions.None);
	}
